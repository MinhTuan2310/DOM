function isMatchSearch(searchTerm, liElement) {
  ...
  return liTitleElement.textContent
    .toLowerCase()
...
function isMatch(params, liElement) {
  return (
    ...
    isMatchSelected(params.get("status"), liElement)
  );
}
...
function createTodoElement(todo, params) {
  ...
  if (divElement) {
    const alertClass =
      liElement.dataset.status === "complete"
        ? "alert-success"
        : "alert-secondary";
    ...
    const buttonContent =
      liElement.dataset.status === "complete" ? "Reset" : "Finish";
    markAsDoneButton.textContent = buttonContent;

    // update button's background color
    const buttonBg =
      liElement.dataset.status === "complete" ? "btn-success" : "btn-dark";
...
function handlerFormSubmit(event) {
  ...
  if (isEdit) {
    ...
    if (liElement) {
      ...
      // update status
      const alertClass =
        currentStatus === "complete" ? "alert-success" : "alert-secondary";
      const buttonBackground =
        currentStatus === "complete" ? "btn-success" : "btn-dark";
